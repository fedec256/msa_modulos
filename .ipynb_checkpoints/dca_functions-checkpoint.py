...
AA_dict = {'-':0,'A':1,'C':2,'D':3,'E':4,'F':5,'G':6,'H':7,'I':8,'K':9,'L':10,'M':11,'N':12,
                  'P':13,'Q':14,'R':15,'S':16,'T':17,'V':18,'W':19,'Y':20}

letters_dict = {value: key for key, value in AA_dict.items()}
...
def MCseq(nsteps: numba.int64,npos: numba.int64 ,Naa: numba.int64,temp: numba.float64,
    ...
    seq=np.array([np.random.randint(0,Naa) for i in range(npos)]) # generate random sequence
    e0=E_tot(seq,Hi,Jij)
    energies=np.zeros(0)
...
def generate_seq_ensemble(path,name_energies, name_seqs, num_cores,Hi,Jij,corr,NSeq,temp=1.0,Naa=21,transient=40000,save_each=5000):
    ...
    nsteps=transient+corr*nseq #ese corr es la cantidad de pasos para que dejen de estar correlacionadas 2 secuencias (funcion q lo calcula existe)
